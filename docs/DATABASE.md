...
### Таблица `stream_session_categories`

Уникальные категории в рамках одной сессии стрима (для выпадающего списка в колонке Category):

- `id` - Уникальный идентификатор (INTEGER PRIMARY KEY)
- `stream_session_id` - ID сессии (INTEGER, FOREIGN KEY)
- `category` - Название категории Twitch (TEXT NOT NULL)
- `first_seen_at` - Когда категория впервые встретилась в этой сессии (INTEGER)
...
1. **Завершение сессии просмотра / офлайн** — при окончательном офлайне, новом stream-up или остановке бота:
   - Баллы текущей сессии (`streamPointsEarned`) добавляются в `total_points` и `daily_points`
   - Состояние сессии (начальный баланс, earned за сессию) дополнительно пишется в `current-points.json` для восстановления после перезапуска

2. **Завершение сессии просмотра (watch time)** — при завершении просмотра стрима:
...
4. **Начало трансляции** (WebSocket stream-up или переход в онлайн через GraphQL):
   ...
   - Текущая категория сохраняется в `stream_session_categories`
