...
**Важно:** База данных является опциональной функцией. Если модуль `better-sqlite3` не может быть установлен или скомпилирован (например, на Android устройствах без Python и build tools), приложение будет работать без базы данных, используя только файловое хранилище (`StatisticsStorage`).
...
Если `better-sqlite3` не может быть установлен (например, на Android без Python и build tools), приложение автоматически отключит функции базы данных и будет работать только с файловым хранилищем (`StatisticsStorage`). Все остальные функции приложения будут работать нормально.

При запуске вы увидите предупреждение:
```
⚠️  better-sqlite3 not available: ... Database features will be disabled.
ℹ️  Database storage not available (better-sqlite3 not installed or not compatible)
```
