...
В UI показываются только параметры, которые нужно задавать вручную. Поля integrity, GQL-заголовки, `TWITCH_USER_ID` и расширенные таймауты по-прежнему можно указать в `config.json` напрямую — они заполняются автоматически (расширение Edge, validate, значения по умолчанию).
